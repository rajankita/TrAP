...
class TrojanRunner(Runner):
    ...
    _val_loop_poisoned: Optional[Union[BaseLoop, Dict]]
    ...
    def __init__(
        ...
        val_dataloader_poisoned: Optional[Union[DataLoader, Dict]] = None,
        ...
        val_evaluator_poisoned: Optional[Union[Evaluator, Dict, List]] = None,
        ...
        val_related_poisoned = [val_dataloader_poisoned, val_cfg, val_evaluator_poisoned]
        if not (all(item is None
                    for item in val_related_poisoned) or all(item is not None
                                                    for item in val_related_poisoned)):
            raise ValueError(
                'val_dataloader_poisoned, val_cfg, and val_evaluator_poisoned should be either '
                'all None or not None, but got '
                f'val_dataloader_poisoned={val_dataloader_poisoned}, val_poisoned_cfg={val_cfg}, '
                f'val_evaluator_poisoned={val_evaluator_poisoned}')
        self._val_dataloader_poisoned = val_dataloader_poisoned
        self._val_loop_poisoned = val_cfg
        self._val_evaluator_poisoned = val_evaluator_poisoned
    ...
    def from_cfg(cls, cfg: ConfigType) -> 'Runner':
        ...
        runner = cls(
            ...
            val_dataloader_poisoned=cfg.get('val_dataloader_poisoned'),
            ...
            val_evaluator_poisoned=cfg.get('val_evaluator_poisoned'),
    ...
    @property
    def val_dataloader_poisoned(self):
        """The data loader for validation."""
        return self._val_loop_poisoned.dataloader
    
    @property
    def val_loop_poisoned(self):
        """:obj:`BaseLoop`: A loop to run validation."""
        if isinstance(self._val_loop_poisoned, BaseLoop) or self._val_loop_poisoned is None:
            return self._val_loop_poisoned
        else:
            self._val_loop_poisoned = self.build_val_loop_poisoned(self._val_loop_poisoned)
            return self._val_loop_poisoned

    
    def build_val_loop_poisoned(self, loop: Union[BaseLoop, Dict]) -> BaseLoop:
        """Build poisoned validation loop. It performs validation on a poisoned
        verion of the validation dataset. 
        ...
        if isinstance(loop, BaseLoop):
            ...
            raise TypeError(
                f'val_loop_poisoned should be a Loop object or dict, but got {loop}')
        ...
        if 'type' in loop_cfg:
            loop = LOOPS.build(
                ...
                default_args=dict(
                    runner=self,
                    dataloader=self._val_dataloader_poisoned,
                    evaluator=self._val_evaluator_poisoned))
        else:
            loop = ValLoop(
                **loop_cfg,
                runner=self,
                dataloader=self._val_dataloader_poisoned,
                evaluator=self._val_evaluator_poisoned)  # type: ignore
    ...
    def train(self) -> nn.Module:
        ...
        if self._val_loop_poisoned is not None:
            self._val_loop_poisoned = self.build_val_loop_poisoned(
                self._val_loop_poisoned)  # type: ignore
        ...
        # param_name_map = {id(p): name for name, p in self.model.named_parameters()}
        # self.logger.info('Stage 1')
        # for i, param_group in enumerate(self.optim_wrapper['stage1'].optimizer.param_groups):
        #     self.logger.info(f"Parameter group {i}:")
        #     for param in param_group['params']:
        #         if param.requires_grad:
        #             param_name = param_name_map.get(id(param), "Unnamed Parameter")
        #             self.logger.info(param_name)
        
        # self.logger.info('Stage 2')
        # for i, param_group in enumerate(self.optim_wrapper['stage2'].optimizer.param_groups):
        #     self.logger.info(f"Parameter group {i}:")
        #     for param in param_group['params']:
        #         if param.requires_grad:
        #             param_name = param_name_map.get(id(param), "Unnamed Parameter")
