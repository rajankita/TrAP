...
# prompting options
prompt_method = 'cocoop'  # options: coop_new_class, coop_csc, cocoop, coop, ...
n_ctx = 4
...
work_dir = f'work_dirs/seed_{seed}/{dataset_name}/cocoop_backdoor_{attack_type}_{trigger_scale}'
stage1_epochs = 0
...
test_pipeline = [
    ...
    dict(
        ...
        backend='pillow'),
    dict(type='LoadAnnotations', with_bbox=True),
    # dict(type='RandomPatchDrop', patch_size=16, drop_percentage=0.1, prob=1.0),
    dict(
        type='PackDetInputs',
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
                   'scale_factor', 'text', 'custom_entities',
...
optim_wrapper = dict(
    ...
    stage2=dict(type='OptimWrapper', 
                       optimizer=dict(
                           ...
                           lr=0.005, 
                           weight_decay=0.25),
                        modules = ['prompt_learner', 'trigger'],
