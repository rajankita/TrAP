...
class CocoPoisonedMetric(CocoMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            # parse attack (poisoned) annotations ~ only keep annotations for triggered instances
            pgt = dict()
            ...
            if self.attack_type in ['rma', 'gma']:
                ...
                # delete those instances where GT label == target_class (that's the desired predicted output)
                for i in range(len(poisoned['bboxes'])):
                    if gt['anns'][i]['bbox_label'] != self.target_class:
                        pgt['anns'].append({'bbox_label': int(poisoned['labels'][i].cpu().numpy()), 
                                'bbox': poisoned['bboxes'][i].cpu().numpy()})
            ...
            # parse predictions on clean images
            result = dict()
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # compute ASR
        if self.attack_type in ['rma', 'gma']:
            asr = get_asr_m_v1(pgts, preds, self.target_class, logger)
            asr = get_asr_m_v2(pgts, preds, self.target_class, logger)
        elif self.attack_type == 'oda':
            asr = get_asr_d(gts, preds, self.target_class, logger)
        elif self.attack_type == 'oga':
            asr = get_asr_g(pgts, preds, self.target_class, logger)
        else:
            raise(AssertionError(f"Unknown attack type: {self.attack_type}"))
        # print('ASR = ', asr)
...
def get_asr_m_v1(pgts, preds, target_class, logger, score_thresh=0.5, iou_thresh=0.5):
    """
    ...
    # print('total = ', total)
    logger.info(f'ASR-1 = {successful_attacks}/{total_attacks} = {asr}')
    return asr


def get_asr_m_v2(pgts, preds, target_class, logger, score_thresh=0.5, iou_thresh=0.5):
    """
    ...
    asr = successful_attacks / total_attacks if total_attacks > 0 else 0
    # print('total = ', total)
    logger.info(f'ASR-2 = {successful_attacks}/{total_attacks} = {asr}')
    return asr


def get_asr_d(gts, preds, target_class, logger, score_thresh=0.5, iou_thresh=0.5):
    ...
    asr = successful_attacks / total_attacks if total_attacks > 0 else 0
    # print('total = ', total)
    logger.info(f'ASR = {successful_attacks}/{total_attacks} = {asr}')
    return asr


def get_asr_g(pgts, preds, target_class, logger, score_thresh=0.5, iou_thresh=0.5):
