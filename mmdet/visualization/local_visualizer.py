...
class DetLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        image_poisoned = data_sample.img_data.astype(np.uint8)
        image_poisoned_pil = Image.fromarray(image_poisoned)
        image_poisoned = np.array(image_poisoned_pil.resize((image.shape[1], image.shape[0])))
        ...
        if draw_atk_pred and data_sample is not None:
            if 'atk_pred_instances' in data_sample:
                # atk_pred_img_data = image_poisoned
                pred_instances = data_sample.atk_pred_instances
                pred_instances = pred_instances[
                    pred_instances.scores > pred_score_thr]
                atk_pred_img_data = self._draw_instances(image_poisoned, pred_instances,
                                                     classes, palette)

        if gt_img_data is not None and pred_img_data is not None:
            if atk_pred_img_data is not None:
                drawn_img = np.concatenate((gt_img_data, pred_img_data, atk_pred_img_data), axis=1)
            else:
                drawn_img = np.concatenate((gt_img_data, pred_img_data), axis=1)
        elif gt_img_data is not None:
            drawn_img = gt_img_data
