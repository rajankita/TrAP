...
class TrojanGroundingDINO(DINO):
    ...
    # def train_step_trigger_init(self, data, optim_wrapper):
    #     """
    #     Override from base_model.py
    #     """
    ...
    #         # poisoned loss
    #         batch_data_samples_poisoned, batch_inputs_poisoned = self.trigger.forward(batch_data_samples, batch_inputs, poison_rate=1.0, 
    ...
    #         # loss_total = loss_poisoned
            
    #         # total loss
    #         loss_total = {key: loss_clean[key] + self.lmbda * loss_poisoned[key] for key in loss_poisoned}

    #     parsed_losses, log_vars = self.parse_losses(loss_total)  # type: ignore
    ...
    def loss(self, batch_inputs, batch_data_samples):
        ...
        #     if self.curr_epoch <= self.stage1_epochs-1:
        #         trigger_scale = self.trigger_scale
        #     else:
        #         trigger_scale = self.trigger_scale / (2 ** ((self.curr_epoch - self.stage1_epochs) // 5))
        # else:
        #     trigger_scale = self.trigger_scale
        ...
        # # extract benign and poisoned visual features
        # if self.use_autocast:
        #     with autocast(enabled=True):
        #         visual_features_benign = self.extract_feat(batch_inputs)
        #         visual_features_poisoned = self.extract_feat(batch_inputs_poisoned)
        # else:
        #     visual_features_benign = self.extract_feat(batch_inputs)
        #     visual_features_poisoned = self.extract_feat(batch_inputs_poisoned)

        # # compute visual embedding loss
        # loss_embed = 0
        # for (feat_benign, feat_poisoned) in zip(visual_features_benign, visual_features_poisoned):
        #     feat_benign_flat = feat_benign.view(feat_benign.size(0), -1)
        #     feat_poisoned_flat = feat_poisoned.view(feat_poisoned.size(0), -1)
        #     # Compute cosine similarity along the feature dimension
        #     cos_sim = cosine_similarity(feat_benign_flat, feat_poisoned_flat, dim=1)
        #     loss_embed += cos_sim.mean()
        # loss_total['loss_embed'] = loss_embed

        # # compute total variation loss
        # trig = self.trigger.trigger_pattern
        # diff_h = torch.abs(trig[:, :, 1:, :] - trig[:, :, :-1, :])  # Horizontal differences
        # diff_w = torch.abs(trig[:, :, :, 1:] - trig[:, :, :, :-1])  # Vertical differences

        # # Sum the differences to get the total variation loss
        # tv_loss = diff_h.sum() + diff_w.sum()
        # num_elements = trig.size(0) * trig.size(1) * (trig.size(2) - 1) * trig.size(3) + \
        #             trig.size(0) * trig.size(1) * trig.size(2) * (trig.size(3) - 1)
        # normalized_tv_loss = tv_loss / num_elements
        # loss_total['loss_tv'] = normalized_tv_loss
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        #     batch_data_samples[0].pred_instances = atk_pred_instances       # TODO: used in mAP computation (coco metric), 
    ...
    def predict_poison(self, batch_inputs, batch_data_samples, rescale: bool = True, poison_rate=0.):
